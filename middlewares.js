...
async function retrieveIngredient(req, res, next) {
    ...
    await axios.get(`https://api.edamam.com/api/nutrition-data?app_id=${process.env.EDAMAM_APP_ID}&app_key=${process.env.EDAMAM_APP_KEY}&ingr=${req.body.ingredient}`)
        .then(response => {
            try{
                ...
                if (data) {
                    ...
                    req.session.ingrList.push(`${data.quantity} ${data.measure === "whole" ? "" : data.measure} ${data.foodMatch}`)
                    req.session.ingredientError = false;
                }
          } catch (error) {
                req.session.ingredientError = true;
...
async function submitRecipe(req, res, next) {
    if(!req.session.ingrList || !req.session.ingrList[0]) {
        ...
        await axios.post(`https://api.edamam.com/api/nutrition-details?app_id=${process.env.EDAMAM_APP_ID}&app_key=${process.env.EDAMAM_APP_KEY}`, {        
        ...
        .then(response => {
            ...
            for (let obj of Object.values(response.data.totalNutrients)) {
                req.session.totalNutrients[obj.label] = `${Math.round(Number(obj.quantity))}${obj.unit}`
            }       
        })
        .catch(err => console.log("An error as occurred. Please review ingredient list", err))
    }
    next()
}
